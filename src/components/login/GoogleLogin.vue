<template>
  <a
    @click="handleClick"
    class="w-full inline-flex space-x-2 cursor-default justify-center py-2 px-4 border border-gray-300 rounded-full shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50"
  >
    <svg
      ...
      GoogleAuth[method]()
        .then((result: any) => {
          Object.keys(result).map((r) => {
            if (result[r].access_token) {
              return this.onSuccess(result[r]);
            }
          });
        })
        .catch((err: any) => {
