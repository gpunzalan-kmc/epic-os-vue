...
export default defineComponent({
  ...
  setup(_, context) {
    ...
    function mutateSide(id: number, event: boolean) {
      selectedWorkPointId.value = id;
      open.value = event;
    }
    watch(
      () => [open.value],
    ...
    async function mergeWorkpoint(childId: number) {
      // console.log(selectedWorkPointId.value);
      await WorkPoints.mergeWorkpoint(selectedWorkPointId.value, [
        childId,
      ]).then(() => {
        ElNotification({
          message: "Device has been set",
